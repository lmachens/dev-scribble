...
const { findCategories, insertGame } = require("./db");
const {
...
function listenSocket(server) {
  ...
  io.on("connection", (socket) => {
    try {
      ...
      socket.on("game settings change", ({ maxRounds, categoryName }) => {
        const { gameId } = getPlayer(playerId);
        setGameSettings(gameId, { categoryName, maxRounds });
        io.to(gameId).emit("game settings change", { maxRounds, categoryName });
      });
      ...
      socket.on("start game", ({ categoryName, maxRounds }) => {
        const { gameId } = getPlayer(playerId);
        setGameSettings(gameId, { categoryName, maxRounds });
