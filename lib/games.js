...
function setGameSettings(gameId, { categoryName, maxRounds }) {
  if (!games[gameId]) {
    return;
  }
  games[gameId].categoryName = categoryName;
...
function getGameWithoutSecret(gameId) {
  ...
  const { nextSecret, timeLeftIntervalId, drawOperations, ...game } = games[
    gameId
  ];
  return game;
}
...
function newRound({
  ...
  setTimeout(() => {
    ...
    game.round++;
    game.nextSecret =
      game.secrets[Math.floor(Math.random() * game.secrets.length)];
...
function distractPlayers({ gameId, playerId, onUpdate }) {
  const game = games[gameId];
  if (!game || !game.distractPossible || !game.nextPlayer) {
    return;
  }
...
exports.setGameSettings = setGameSettings;
