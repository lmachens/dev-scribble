...
async function createGame(gameId, playerId, playerName) {
  const categoriesCollection = await getCategoriesCollection();
  const category = await categoriesCollection.findOne({ name: "Deutsch" });
  games[gameId] = {
    gameId,
    owner: {
      id: playerId,
...
function distractPlayers({ gameId, playerId, onUpdate }) {
  const game = games[gameId];
  if (!game || !game.distractPossible || !game.nextPlaye) {
    return;
  }
